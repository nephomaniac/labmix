...
def wait_for_result(callback,
    ...
    debug("Beginning poll loop for result " + str(callback.__name__) + " to go to " +
          str(result))
    start = time.time()
    elapsed = 0
    ...
    while (elapsed < timeout and not oper(current_state, result)):
        debug(str(callback.__name__) + ' returned: "' + str(current_state) + '" after ' +
    ...
    if not oper(current_state, result):
        raise TimeoutFunctionException(str(callback.__name__) + " did not return " +
