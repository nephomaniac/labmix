"""
This is the base class for any test case to be included in the LabTest repo. It should include any
...
from lab_utils.log_utils import markup, ForegroundColor, BackGroundColor, TextStyle
from lab_utils.log_utils import red, green, blue, yellow, cyan, get_traceback, get_terminal_size
from io import StringIO
import copy
from test_utils.testconfig import TestConfig
...
class TestUnit(object):
    ...
    def create_testcase_from_method(cls, method, test_logger=None, eof=False, *args, **kwargs):
        '''
        Description: Creates a TestUnit object from a method and set of arguments to be
        fed to that method
    ...
    def info(self):
        if self._info is None:
            ...
            try:
                info['file'] = self.method.__func__.__code__.co_filename
    ...
    def get_test_method_description(self, header=True):
        ...
        if info_desc:
            ...
            try:
                ...
                if not doc:
                    try:
                        desc = desc + "\n".join(self.method.__func__.__doc__.title().splitlines())
                    except:
                        pass
                    return desc
    ...
    def run(self, eof=None):
        ...
        try:
            ...
            return ret
        except SkipTestException as se:
...
#  - a common CLI
#  - a common CLI arguments used when testing a cloud environment with LabTest.
...
class CliTestRunner(object):
    ...
    _DEFAULT_CLI_ARGS = {
        ...
        'password': {'args': ["--password"],
                     'kwargs': {"help": "Password to use for machine root ssh access",
                                "default": None}},
        'cpe_type': {'args': ["--cpe-type"],
                   'kwargs': {"help": "CPE Type to use in this test",
                              "default": "smartos"}},
        'gw_cpe': {'args': ["--gw-cpe"],
                'kwargs': {"help": "Address of the CPE acting as the gateway or hub",
                           "default": None}},
        ...
        'environment_file': {'args': ['--environment-file'],
                             'kwargs': {"help": "Environment file that describes test topology",
                                        "default": None}},
    ...
    def __init__(self, name=None, description=None, **kwargs):
        ...
        for argname, arg_dict in list(self._DEFAULT_CLI_ARGS.items()):
        ...
        self.log = LabLogger(identifier=self.name, stdout_level=log_level,
    ...
    def get_default_userhome_config(self, fname='labtest.conf'):
        '''
        Description: Attempts to fetch the file 'fname' from the current user's home dir.
        Returns path to the user's home dir default labtest config file.

        :type fname: string
        :param fname: the labtest default config file name

        :rtype: string
        :returns: string representing the path to 'fname', the default labtest conf file.
        '''
        try:
            def_path = os.getenv('HOME') + '/.labtest/' + str(fname)
        except:
            return None
        try:
            os.stat(def_path)
    ...
    def create_testunit_from_method(self, method, *args, **kwargs):
        '''
        Description: Convenience method calling TestUnit.
                     Creates a TestUnit object from a method and set of arguments to be
                     fed to that method
    ...
    def create_testunit_by_name(self, name, obj=None, eof=True, autoarg=True, test_logger=None,
        ...
        TestUnit object from that method and the provided positional as well as keyword
        arguments provided.
    ...
    def populate_testunit_with_args(self, testunit, namespace=None):
        ...
        :type: testunit: cli_test_runner.TestUnit object
    ...
    def dump_test_info_labtest(self, testlist=None, filepath=None, printresults=True):
        testlist = testlist or self._testlist
        if not testlist:
            self.log.warning('Test runlist is empty')
    ...
    def handle_dry_run(self, testlist, printresults):
        ...
        if dry_run_arg is not False:
            ...
            if isinstance(dry_run_arg, (str,bytes)):
                ...
                try:
                    ...
                    if fmt == 'yaml':
                        ...
                        handler = self.dump_test_info_labtest
                    elif fmt == 'json':
                        handler = self.dump_test_info_json
                    else:
                        raise ValueError('Unknown format for dry_run:"{0}". Supported Values:'
                                         '"json, yaml, labtest"'.format(args[0]))
    ...
    def run(self, testlist=None, eof=False, clean_on_exit=None, test_regex=None,
        ...
        :type list: list
        :param list: list of TestUnit objects to be run

        :type eof: boolean
        :param eof: Flag to indicate whether run_test_case_list should exit on any failures.
                    If this is set to False it will exit only when a given TestUnit
    ...
    def get_args(self, use_cli=True, file_sections=[], runtime_kwargs=None, verbose=True):
        ...
        through local testcase's 'argparse' methods, as well as retrieve all TestConfig file
        ...
        :param use_default_files: Boolean to indicate whether or not to read default config file
                                 at $HOME/.labtest/labtest.conf (not indicated by cli)

        :type sections: list
        :param sections: list of TestConfig sections to read configuration values from, and store
                         in self.args.
        ...
        # create testconfig configparser objects from each file.
        if config_file:
            self.config_file = TestConfig(filename=configfile)
        ...
        if cliargs:
            ...
            args = cf
        for arg_name, value in list(runtime_kwargs.items()):
    ...
    def get_method_fcode(cls, meth):
        ...
        if isinstance(meth, object):
            try:
                f_code = meth.__init__.__func__.__code__
            except:
                pass
                # Check for instance method...
        if isinstance(meth, types.MethodType):
            try:
                f_code = meth.__func__.__code__
            except:
                pass
                # Check for function...
        if isinstance(meth, types.FunctionType):
            try:
                f_code = meth.__code__
            except:
                pass
        if not f_code:
            raise Exception(
    ...
    def print_test_list_results(self, testlist=None, descriptions=False,
        ...
        '''
        Description: Prints a formated list of results for a list of cli_test_runner.TestUnits

        :type testlist: list
        :param testlist: list of cli_test_runner.TestUnits
    ...
    def print_test_list_short_stats(self, list, printmethod=None):
        ...
        for result_string in [x for x in dir(TestResult) if not x.startswith('__')]:
